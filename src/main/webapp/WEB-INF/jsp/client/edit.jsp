 ...
 <script type="text/javascript">
  ...
  $("#clientForm").validate({  // <-- initialize plugin on the form.
   ...
   <c:if test="${fn:length(errors) gt 0}">
        ...
        <form id="clientForm" action="${pageContext.request.contextPath}/client/edit" method="POST" class="form-horizontal" role="form">
           ...
           <div class="form-group">
            ...
            <div class="col-sm-8 col-sm-offset-1">
            	<p>
            		<b>Current Contacts:</b> (Select to Remove)
            	</p>
            </div>
			<div class="form-group">
            <div class="col-sm-4 col-sm-offset-2">      
         		<c:choose>
        			<c:when test="${not empty client.contacts}">
          				<c:forEach items="${client.contacts}" var="contact">
          					<div class="checkbox">
          						<label><input type="checkbox" name="removeIds" value="${contact.personId }">${contact.firstName} ${contact.lastName }</label>
                			</div>
                		</c:forEach>
            		</c:when> 
            		<c:otherwise>
            			Client currently has no Contacts.
            		</c:otherwise>
            	</c:choose>
            </div>
            </div>
            ...
            <div class="col-sm-8 col-sm-offset-1">
            	<p>
            		<b>Available Contacts:</b> (Select to Add)
            	</p>
            </div>
            <div class="form-group">
                ...
                <c:choose>
        			<c:when test="${not empty people}">
                			...
                			<div class="checkbox">
                 				<label><input type="checkbox" name="addIds" value="${person.personId }">${person.firstName} ${person.lastName }</label>
               				 </div>
               				 </c:forEach>
